/****************************************************************************
** Meta object code from reading C++ file 'feedbackdialog.h'
**
** Created: Thu Apr 6 20:32:37 2017
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "feedbackdialog.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'feedbackdialog.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_FeedbackDialog[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      16,   15,   15,   15, 0x08,
      41,   15,   15,   15, 0x08,
      53,   15,   15,   15, 0x08,
      74,   68,   15,   15, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_FeedbackDialog[] = {
    "FeedbackDialog\0\0NetReply(QNetworkReply*)\0"
    "ShowSpecs()\0SendFeedback()\0reply\0"
    "finishedSlot(QNetworkReply*)\0"
};

void FeedbackDialog::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        FeedbackDialog *_t = static_cast<FeedbackDialog *>(_o);
        switch (_id) {
        case 0: _t->NetReply((*reinterpret_cast< QNetworkReply*(*)>(_a[1]))); break;
        case 1: _t->ShowSpecs(); break;
        case 2: _t->SendFeedback(); break;
        case 3: _t->finishedSlot((*reinterpret_cast< QNetworkReply*(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData FeedbackDialog::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject FeedbackDialog::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_FeedbackDialog,
      qt_meta_data_FeedbackDialog, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &FeedbackDialog::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *FeedbackDialog::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *FeedbackDialog::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_FeedbackDialog))
        return static_cast<void*>(const_cast< FeedbackDialog*>(this));
    return QDialog::qt_metacast(_clname);
}

int FeedbackDialog::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
