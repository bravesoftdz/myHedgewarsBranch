/****************************************************************************
** Meta object code from reading C++ file 'pagetraining.h'
**
** Created: Thu Apr 6 20:32:44 2017
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "pagetraining.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'pagetraining.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_PageTraining[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      25,   14,   13,   13, 0x05,

 // slots: signature, parameters, type, tag, flags
      47,   13,   13,   13, 0x08,
      63,   13,   13,   13, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_PageTraining[] = {
    "PageTraining\0\0scriptName\0startMission(QString)\0"
    "startSelected()\0updateInfo()\0"
};

void PageTraining::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        PageTraining *_t = static_cast<PageTraining *>(_o);
        switch (_id) {
        case 0: _t->startMission((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 1: _t->startSelected(); break;
        case 2: _t->updateInfo(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData PageTraining::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject PageTraining::staticMetaObject = {
    { &AbstractPage::staticMetaObject, qt_meta_stringdata_PageTraining,
      qt_meta_data_PageTraining, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &PageTraining::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *PageTraining::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *PageTraining::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_PageTraining))
        return static_cast<void*>(const_cast< PageTraining*>(this));
    return AbstractPage::qt_metacast(_clname);
}

int PageTraining::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = AbstractPage::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void PageTraining::startMission(const QString & _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
